...
void test_get_pos() {
  ...
  assertDoubleEqual(math_vector_length(target.getPosition()), 30, 1e-4);
  assertIntEqual( target.getRCS(), 8.0 );

  target.setPosition({3, 4, 0});
  assertDoubleEqual( math_vector_length(target.getPosition()), 5, 1e-4 ); 
}
...
void test_path() {
  ...
  assertTrue( target.getPosition(0) == start );

  assertTrue( target.getPosition(0.5) == middle );
}
...
void test_move() {
  ...
  assertTrue( ptr_entry == copy.getPath().getEntryVector().data() );
  assertTrue( ptr_array == copy.getPath().getValueVector()[0].data() );
