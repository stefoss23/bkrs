...
void test_1() {
  ...
  math_vector a = {2, 2, 1};
  assertDoubleEqual( math_vector_length(a), 3, 1e-4 );
  math_vector b = {1, 2, 3};
  assertDoubleEqual( a * b, 9, 1e-4 );

  a = {1, 0, 0};
  b = {0, 1, -1};
  assertDoubleEqual( math_vector_angle(a, b), 0.5 * pi, 1e-4 );

  b = {-1, 1, 0}; 
  assertDoubleEqual( math_vector_angle(a, b), 0.75 * pi, 1e-4  );

  a = {1.5, 2.25, 8.75};
  assertTrue( ((a / 2.0) == (math_vector){0.75, 1.125, 4.375}) );

  a = math_vector_unit({3, 4, 0});
  assertDoubleEqual( a[0], 0.6, 1e-4 );
  assertDoubleEqual( a[1], 0.8, 1e-4 );
  assertTrue( a[2] == 0 );

  a = math_vector_unit({45, 789, 11});
  assertDoubleEqual( math_vector_length(a), 1.0, 1e-4 );
  ...
  assertDoubleEqual( a[0] , 0.666667, 1e-3 );
  assertDoubleEqual( a[1] , 0.666667, 1e-3 );
  assertDoubleEqual( a[2] , 0.333333, 1e-3 );
  ptr2 = a.data();
  assertTrue( ptr1 == ptr2 );
...
void test_transform() {
  ...
  assertTrue( math_vector_equal(t1, t2, 1e-4) );
  math_vector t3 = {0.707, 0.707, 0};
  math_vector_rotate_xy(t2, 135 * deg_to_rad);
  assertTrue( math_vector_equal(t2, t3, 1e-2) );
}
...
void test_spherical() {
  ...
  assertTrue( math_vector_equal(x_, x, 1e-4) );
  assertTrue( math_vector_equal(y_, y, 1e-4) );
  assertTrue( math_vector_equal(z_, z, 1e-4) );
}
